...
# ═══════════════════════════════════════════════════════════════════════════════
#  DRIVE DETECTION  — from Code 2 (GetDiskFreeSpaceExW)
#  This is what makes D:\\ and other secondary drives show up correctly.
#  Code 1 used os.path.exists() which sometimes misses real drives.
# ═══════════════════════════════════════════════════════════════════════════════
def get_drives():
    drives  = []
    ...
    for letter in "ABCDEFGHIJKLMNOPQRSTUVWXYZ":
        if bitmask & 1:
            ...
            # GetDiskFreeSpaceExW actually tests if the drive is accessible
            # (from Code 2 — handles D:\\ that os.path.exists misses)
            try:
...
# ── Registry helpers — HKLM + HKCU (from Code 1) ─────────────────────────────
...
# ═══════════════════════════════════════════════════════════════════════════════
#  EXPLORER CONTROL  — from Code 1 (wait-for-dead / wait-for-alive)
# ═══════════════════════════════════════════════════════════════════════════════
# Shell notification constants — combined from BOTH codes
...
def kill_explorer():
    """Kill Explorer and wait until process is fully gone."""
...
def start_explorer():
    """Start Explorer and wait until it's running and settled."""
    ...
    for _ in range(40):
        ...
        if "explorer.exe" in r.stdout.lower():
            break
        time.sleep(0.3)
    time.sleep(2.0)   # let Explorer fully settle before sending notifications

def notify_shell(drive):
    """
    Combined notifications from BOTH codes:
    - Code 1: UPDATEITEM + ASSOCCHANGED + ALLEVENTS
    - Code 2: adds RENAMEFOLDER (extra kick for D:\\ and secondary drives)
    """
...
def full_refresh_quick(drive):
    """Quick refresh without killing Explorer — for remove/diagnostics."""
    try:
...
# ═══════════════════════════════════════════════════════════════════════════════
#  APPLY PIPELINE  v6.0 — MERGED ORDER
#
#  Uses Code 1's KILL-FIRST order (makes C:\\ work) combined with
#  Code 2's ie4uinit + RENAMEFOLDER (makes D:\\ and others work):
#
#  1. Kill Explorer          → releases ALL file locks (Code 1)
#  2. Delete icon cache      → files unlocked, actually deletable (Code 1)
#  3. Delete old .ico files  → no stale icon remains (Code 1)
#  4. ie4uinit -show         → pre-clear before copy (Code 2 idea)
#  5. Copy NEW .ico          → fresh file, Explorer not running (Code 1)
#  6. Write Registry         → HKLM + HKCU (Code 1)
#  7. Write desktop.ini      → network visibility (Code 1)
#  8. ie4uinit -ClearCache   → post-write cache clear (Code 2)
#  9. Start Explorer         → loads fresh icon (Code 1)
# 10. Notify shell x3        → UPDATEITEM+RENAMEFOLDER+ASSOCCHANGED (merged)
# ═══════════════════════════════════════════════════════════════════════════════
def apply_icon_pipeline(drive, ico_src, label, hide_files, do_eject,
    ...
    try:
        ...
        # ── 1. Kill Explorer (Code 1 approach) ───────────────────────────────
        ...
        # ── 4. ie4uinit pre-clear (Code 2 idea — before writing new file) ────
        ...
        # ── 7. desktop.ini — makes icon visible from other computers ──────────
        ...
        # ── 8. autorun.inf for USB (Code 2 approach) ──────────────────────────
        if is_usb:
        ...
        # ── 9. ie4uinit post-write + start Explorer ───────────────────────────
        ...
        # ── 10. Notify shell — triple fire with RENAMEFOLDER (merged) ─────────
...
class StepLog(tk.Toplevel):
    def __init__(self, parent):
        ...
        tk.Label(self, text="Live Progress", bg=BG, fg=ACCENT,
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Drive Icon Setter  v2.0")
    ...
    def _build_ui(self):
        tk.Label(self,text="  Drive Icon Setter  v6.0",bg=BG,fg=ACCENT,
                 font=("Segoe UI",15,"bold")).pack(anchor="w",pady=(0,4))

        info=tk.Frame(self,bg=SURFACE,padx=12,pady=8)
        info.pack(fill="x",pady=(0,6))
        tk.Label(info,
                 text="Kill-first pipeline + GetDiskFreeSpaceExW detection",
                 bg=SURFACE,fg=GREEN,
    ...
    def _apply(self):
        ...
        # Show progress bar (from Code 2) + StepLog window (from Code 1)
