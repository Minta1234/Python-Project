"""
Drive Icon Setter  v6.1
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
FIXED from Document 4:
  ❌ REMOVED: taskkill dllhost.exe     (crashed COM+ / system services)
  ❌ REMOVED: taskkill ShellExperienceHost.exe  (broke Windows 11 UI)
  ✅ KEPT:    Windows 10/11 detection
  ✅ KEPT:    Win11 extra cache paths
  ✅ KEPT:    All v6.0 safe pipeline logic
"""
...
# Cache paths — Win11 has extra locations
ICO_CACHE_PATHS = [
...
if IS_WIN11:
    ICO_CACHE_PATHS += [
        r"%LOCALAPPDATA%\Microsoft\Windows\Explorer\*.db",
    ]
...
def get_drives():
    ...
    for letter in "ABCDEFGHIJKLMNOPQRSTUVWXYZ":
        if bitmask & 1:
            ...
            try:
                # GetDiskFreeSpaceExW actually tests accessibility (fixes D:\)
...
# ═══════════════════════════════════════════════════════════════════════════════
#  EXPLORER CONTROL  — SAFE for Win10 and Win11
#  ✅ Only kills explorer.exe (safe)
#  ❌ Never kills dllhost.exe        (would crash COM+ services)
#  ❌ Never kills ShellExperienceHost (would break Win11 taskbar/Start)
# ═══════════════════════════════════════════════════════════════════════════════
def kill_explorer():
    """Kill ONLY explorer.exe — nothing else."""
...
def delete_icon_cache():
    """Delete icon cache DB files while Explorer is dead."""
    deleted = 0
    for pattern in ICO_CACHE_PATHS:
        for f in glob.glob(os.path.expandvars(pattern)):
            try:
                ...
                pass
    # ie4uinit while Explorer is still dead helps on Win11
    try:
...
def start_explorer():
    """Start explorer.exe and wait for it to be ready."""
    ...
    for _ in range(40):
        ...
        if "explorer.exe" in r.stdout.lower():
            break
        time.sleep(0.3)
    # Win11 needs a bit more settle time
    time.sleep(2.5 if IS_WIN11 else 2.0)

def notify_shell(drive):
    """
    Safe shell notifications — no process kills.
    Sends all signal types that trigger Explorer drive icon redraw.
    """
    ...
    # Safe ie4uinit kick (no process kill)
    try:
...
def full_refresh_quick(drive):
    """Quick refresh for Remove Icon — safe on Win10 and Win11."""
    try:
        ...
        pass
    notify_shell(drive)
    # Only kill explorer.exe — never other system processes
    subprocess.run(["taskkill", "/F", "/IM", "explorer.exe"],
                   shell=True, capture_output=True)
    time.sleep(2.0 if IS_WIN11 else 1.5)
    subprocess.Popen("explorer.exe", shell=True)
    time.sleep(1.5 if IS_WIN11 else 1.0)
    notify_shell(drive)
...
# ═══════════════════════════════════════════════════════════════════════════════
#  APPLY PIPELINE  v6.1
#  Safe 10-step pipeline — works on Win10 and Win11
#  Never kills system processes (dllhost, ShellExperienceHost etc.)
# ═══════════════════════════════════════════════════════════════════════════════
def apply_icon_pipeline(drive, ico_src, label, hide_files, do_eject,
    ...
    try:
        ...
        # ── 1. Kill Explorer (ONLY explorer.exe) ─────────────────────────────
        step(f"[1/10] Stopping Explorer (Windows {WIN_VERSION})...")
        kill_explorer()
        ...
        # ── 4. ie4uinit pre-clear (before writing new file) ───────────────────
        ...
        # IMPORTANT: Always register the ProgramData path, NOT the drive root path.
        # ico_root (E:\drive_icon.ico) gets hidden with +H +S later —
        # Windows CANNOT load hidden files as icons, causing blank icon on D:\, E:\ etc.
        # ico_dest (C:\ProgramData\DriveIcons\drive_E.ico) is never hidden = always works.
        step("[6/10] Writing Windows Registry (HKLM + HKCU)...")
        reg_set_icon(drive, ico_dest, label)
        step(f"[6/10] Registry → {ico_dest}")
        step(f"[6/10] Registry OK: DriveIcons\\{letter}\\DefaultIcon")

        # ── 7. desktop.ini — icon visible from other computers ────────────────
        ...
        # ── 9. Post-write cache rebuild + start Explorer ──────────────────────
        step("[9/10] Rebuilding icon cache (ie4uinit)...")
        try:
            subprocess.run(["ie4uinit.exe", "-ClearIconCache"],
                           capture_output=True, timeout=10)
        except:
            pass
        subprocess.run(["ie4uinit.exe", "-show"], capture_output=True)
        step(f"[9/10] Starting Explorer (Windows {WIN_VERSION})...")
        start_explorer()

        # ── 10. Notify shell — triple fire ────────────────────────────────────
        step("[10/10] Notifying shell (x3)...")
        notify_shell(drive)
        time.sleep(1.0)
        notify_shell(drive)
        time.sleep(0.5)
        ...
        total = time.time() - t0
        step(f"[DONE] Finished in {total:.1f}s (Windows {WIN_VERSION})")
        ...
        if eject_ok:
            done_cb(True,
                    f"Icon applied to {drive}  (Win {WIN_VERSION})\n"
...
def drive_diagnostics(drive):
    ...
    lines  = [f"=== Diagnostics: {drive} ===",
              f"Windows: {WIN_VERSION} ({'Win11' if IS_WIN11 else 'Win10'})"]
    ...
    cache_count = sum(
        len(glob.glob(os.path.expandvars(p))) for p in ICO_CACHE_PATHS)
    lines.append(f"Cache files    : {cache_count}")
...
class StepLog(tk.Toplevel):
    def __init__(self, parent):
        ...
        self.title(f"Progress — Windows {WIN_VERSION}")
        self.configure(bg=BG, padx=16, pady=14)
        self.resizable(False, False)
        self.protocol("WM_DELETE_WINDOW", lambda: None)
        tk.Label(self, text=f"Live Progress  (Windows {WIN_VERSION})",
                 bg=BG, fg=ACCENT,
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        opt_txt = "Windows 11 mode — extra cache cleared" if IS_WIN11 \
                  else "Windows 10 mode"
        tk.Label(info,
                 text=f"Safe pipeline — {opt_txt}",
                 bg=SURFACE,fg=GREEN,
                 font=("Segoe UI",9,"bold")).pack(anchor="w")
        tk.Label(info,
                 text="C:\\ + D:\\ + E:\\ + ALL drives | Registry + desktop.ini + network",
